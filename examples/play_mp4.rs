...
use bevy_movie_player::mp4::Mp4Movie;
use bevy_movie_player::plugin::MoviePlayerPlugin;
...
pub struct MovieAssets {
  #[asset(path = "test.mp4")]
  pub test: Handle<Mp4Movie>,

  // // if you want to load on memory:
  // #[asset(path = "test.mp4")]
  // pub test: Handle<Mp4MovieOnMemory>,
}
...
fn setup(
    ...
    mut assets: ResMut<Assets<Mp4Movie>>,
    // mut asset_server: Res<AssetServer>,
    // time: Res<Time>,
) {
    let movie = assets.get_mut(movie_assets.test.id()).unwrap();
